...
draft: false
tags: ["Paper Reading", "Recommendation System", "Transformer", "Ranking", "MoE", "Generative Pretraining", "SORT"]
categories: ["Tech"]
showToc: true
TocOpen: true
---
...
{{< mermaid >}}
graph TD
    Problem["核心矛盾<br/>高特征稀疏性 × 低标签密度"]
    
    subgraph Solutions ["SORT 系统性解决方案"]
        S1["🔧 请求中心样本组织<br/>减少冗余计算"]
        S2["🔧 特殊 Token 设计<br/>稳定注意力分布"]
        S3["🔧 稀疏注意力 + 查询裁剪<br/>降低复杂度 + 引入归纳偏置"]
        S4["🔧 DeepSeek MoE FFN<br/>扩大容量不增计算"]
        S5["🔧 生成式预训练 + 嵌入冻结<br/>解决稀疏参数过拟合"]
        S6["🔧 QKNorm + 门控注意力<br/>稳定训练过程"]
    end
...
{{< mermaid >}}
graph TD
    subgraph Traditional ["传统方法：物品中心的样本组织"]
        R1["用户请求<br/>N 个候选物品"]
        
        S1["样本 1<br/>History + Profile + Item₁"]
        S2["样本 2<br/>History + Profile + Item₂"]
        S3["样本 3<br/>History + Profile + Item₃"]
        SN["样本 N<br/>History + Profile + Itemₙ"]
        ...
        Note1["⚠️ History 和 Profile<br/>被重复编码 N 次"]
    end
    
    subgraph SORT_Method ["SORT：请求中心的样本组织"]
        R2["用户请求<br/>N 个候选物品"]
        
        US["统一样本 S = ⟨H, U, C⟩<br/>History + Profile + {Item₁, Item₂, ..., Itemₙ}"]
        
        R2 --> US
        
        FF["单次前向传播<br/>同时处理 N 个候选"]
        
        US --> FF

        Note2["✅ History 和 Profile<br/>仅编码 1 次"]
    end
{{< /mermaid >}}
...
graph TD
    ...
    subgraph Layer1 ["浅层 (Layer 1-4)"]
        L1["所有 Token 保留 Q/K/V<br/>局部注意力窗口 w=256"]
    end

    subgraph Layer2 ["中间层 (Layer 5-8)"]
        L2["裁剪远距离 History Token 的 Q<br/>保留 K/V 供候选物品访问"]
    end

    subgraph Layer3 ["深层 (Layer 9-12)"]
        L3["仅保留 ≤128 个 History Q<br/>+ 所有 Candidate Q/K/V"]
    end

    subgraph Output ["输出层"]
        O1["Candidate Token 隐层输出"]
        O2["FFN Head → CTR/CVR 预测"]
    end
...
graph TD
    subgraph Stage1 ["阶段一：生成式预训练"]
        A["用户历史行为序列<br/>[i₁, i₂, ..., iₙ]"]
        B["Transformer Decoder"]
        C["Next-Item Prediction<br/>(Sampled Softmax)"]
        D["预训练物品 Embedding 表<br/>E_pretrained"]
    ...
    subgraph Transfer ["迁移策略"]
        D -->|"复制参数"| E["冻结的 Embedding 表 ❄️<br/>不参与梯度更新"]
    end
    
    subgraph Stage2 ["阶段二：判别式排序训练"]
        F["请求样本 S = ⟨H, U, C⟩"]
        G["Token 化"]
        H["SORT Transformer<br/>(仅稠密参数可训练 🔥)"]
        I["排序头 FFN"]
        J["CTR / CVR 预测"]
