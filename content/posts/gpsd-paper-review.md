...
draft: false
tags: ["Paper Reading", "Recommendation System", "Transformer", "Scaling Law", "Generative Pretraining"]
categories: ["Tech"]
---
...
{{< mermaid >}}
graph TD
    subgraph Stage 1: Generative Pretraining
        A[用户历史行为序列] --> B(Transformer Decoder)
        B --> C[预测下一个 Item (Sampled Softmax)]
        C --> D{预训练模型}
    end

    subgraph Stage 2: Bridging Strategy
        D -->|提取| E[稀疏参数: Embeddings]
        D -->|提取| F[稠密参数: Transformer]
    end

    subgraph Stage 3: Discriminative Training (CTR/CVR)
